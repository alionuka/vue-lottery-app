<template>
  <div class="container my-4">
    ...
    <CustomModal v-if="showDeleteModal" @close="closeDeleteModal">
      ...
      <template #body>
        ...
        <div class="d-flex justify-content-end">
          <button class="btn btn-secondary me-2" @click="closeDeleteModal">
            Ні
          </button>
    ...
    <!-- Модальне вікно редагування переможця -->
    ...
    <!-- Модальне вікно успішного додавання переможця -->
...
export default {
  ...
  setup() {
    ...
    // Завантаження переможців з localStorage
    const loadWinners = () => {
    ...
    // Зберігання переможців в localStorage при зміні
    watch(
      winners,
      (newWinners) => {
    ...
    // Ініціалізація збережених даних
    onMounted(() => loadWinners());

    // Додавання нового переможця
    ...
    const updateWinner = () => {
      const index = winners.value.findIndex(
        (w) => w.email === editForm.value.email
      );
    ...
    const selectRandomWinner = () => {
      if (winners.value.length === 0 || selectedWinners.value.length >= 3)
        return;
    ...
    // Фільтрування переможців по імені
